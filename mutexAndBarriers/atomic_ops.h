...
static inline void tatas_acquire_slowpath_withoutBackOff(tatas_lock_t *L)
{
    int b = 64;
    while (tas(L))
    {
...
static inline void tatas_acquire_withoutBackOff(tatas_lock_t *L)
{
    if (tas(L))
        tatas_acquire_slowpath_withoutBackOff(L);
}
